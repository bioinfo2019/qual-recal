...
bool isGoodRead(BamAlignmentRecord record, int refContigLength, std::string refContigName)
{
	...
	{
		if((fabs(read1_positions[i] - beginPos) < 2) && read1Chromosome == refContigName)
			return true;
	}

	for(int i = 0;i < read2_positions.size();++i)
	{
		if((fabs(read2_positions[i] - beginPos) < 2) && read2Chromosome == refContigName)
			return true;
	}
